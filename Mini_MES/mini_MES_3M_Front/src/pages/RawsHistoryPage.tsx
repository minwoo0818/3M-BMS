...
const INBOUND_SEARCH_OPTIONS: SearchOption[] = [
  { key: "전체", label: "전체" },
  { key: "vendorName", label: "매입처명" },
  { key: "itemName", label: "품목명" },
  { key: "itemCode", label: "품목번호" },
  { key: "inboundNo", label: "입고 번호" },
  { key: "inboundDate", label: "입고 일자" },
...
const OUTBOUND_SEARCH_OPTIONS: SearchOption[] = [
  ...
  { key: "itemName", label: "품목명" },
  { key: "itemCode", label: "품목번호" },
  { key: "outboundNo", label: "출고 번호" },
  { key: "outboundDate", label: "출고 일자" },
  { key: "manufacturer", label: "제조사" },
];
...
// 출고 더미 데이터 (28개)
const DUMMY_OUTBOUND_DATA: OutboundItem[] = Array.from(
  { length: 28 },
  (_, i) => {
...
export default function App() {
  ...
  const { title, placeholder, searchOptions, dummyData } = useMemo(() => {
    if (currentHistoryType === "INBOUND") {
      return {
        ...
        placeholder:
          "매입처명, 품목명, 품목번호, 입고 번호, 입고 일자로 검색해주세요",
      ...
      return {
        ...
        placeholder:
          "매입처명, 품목명, 품목번호, 출고 번호, 출고 일자로 검색해주세요",
    ...
    (e: React.FormEvent) => {
      ...
      const filtered = dummyData.filter((item) => {
        ...
        if (searchKey === "전체") {
          ...
          // 전체 검색 필드 통합
          searchKeyStr = `${item.vendorName} ${item.itemCode} ${
            item.itemName
          } ${item.itemSpec} ${item.manufacturer} ${
            itemAsInbound.inboundNo || ""
          } ${itemAsOutbound.outboundNo || ""} ${
            itemAsInbound.inboundDate || ""
          } ${itemAsOutbound.outboundDate || ""} ${
            itemAsInbound.manufactureDate || ""
          }`;
        } else {
          // 개별 검색 키 처리
          if (searchKey.includes("Date")) {
            searchKeyStr = isOutbound
              ? (item as OutboundItem).outboundDate
              : (item as InboundItem).inboundDate;
          } else if (searchKey.includes("No")) {
            searchKeyStr = isOutbound
              ? (item as OutboundItem).outboundNo
              : (item as InboundItem).inboundNo;
          } else {
  ...
  const renderTableBody = () => {
    ...
    return currentData.map((item, index) => {
      ...
      const uniqueKey = `${currentHistoryType}-${
        (item as InboundItem).inboundNo || (item as OutboundItem).outboundNo
      }-${item.no}`;
      ...
      return (
        ...
        >
          {/* No. */}
          <td style={{ ...styles.td, ...styles.tdCenter, width: "3%" }}>
            {displayNo}
          </td>
          ...
          {/* 품목 번호 */}
          <td style={{ ...styles.td, width: "8%", ...styles.tdCenter }}>
            {item.itemCode}
          </td>

          {/* 품목명 */}
          <td style={{ ...styles.td, width: "12%", fontSize: "13px" }}>
            {item.itemName}
          </td>
          ...
          <td
            style={{
              ...
              ...styles.tdRight,
              width: "7%",
              color: isOutbound ? "#b91c1c" : "#059669",
            }}
          >
          ...
          {/* 입고/출고 일자 */}
          <td style={{ ...styles.td, ...styles.tdCenter, width: "8%" }}>
            {isOutbound
              ? itemAsOutbound.outboundDate
              : itemAsInbound.inboundDate}
          </td>
      ...
      >
        ...
        <button
          ...
          onClick={() =>
            console.log(
              `${
                currentHistoryType === "INBOUND" ? "입고" : "출고"
              } 데이터 Excel 다운로드 실행`
            )
          }
        >
        ...
        <table style={styles.table}>
          <thead>
            <tr>
              ...
              <th style={styles.th(false, false)}>
                {currentHistoryType === "INBOUND" ? "입고 번호" : "출고 번호"}
              </th>
              ...
              <th style={styles.th(false, false)}>
                {currentHistoryType === "INBOUND" ? "입고 수량" : "출고 수량"}
              </th>

              {currentHistoryType === "INBOUND" && (
                <th style={styles.th(false, false)}>총량(규격X수량)</th>
              )}

              <th style={styles.th(false, false)}>
                {currentHistoryType === "INBOUND" ? "입고 일자" : "출고 일자"}
              </th>

              {currentHistoryType === "INBOUND" && (
                <th style={styles.th(false, false)}>제조 일자</th>
              )}
